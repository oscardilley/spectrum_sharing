...
#     step = u / tf.abs(v)**(1/2)
#     # Scale to get movements between 1-5 grid spaces
#     step = 3.0 + (step * 2.0)  # Center around 3 with ±2 variation
#     return tf.clip_by_value(step, 1.0, 6.0)

# def find_valid_position(grid, base_pos):
#     """Find the nearest valid position to the given base position"""
#     y_max, x_max = grid.shape
#     radius = 0
#     base_pos = tf.cast(base_pos, tf.int64)
    
#     while radius < max(y_max, x_max):
#         # Try positions in increasing radius until valid position found
#         for dy in range(-radius, radius + 1):
#             for dx in range(-radius, radius + 1):
#                 pos = base_pos + tf.constant([dy, dx], dtype=tf.int64)
#                 if (pos[0] >= 0 and pos[0] < y_max and 
#                     pos[1] >= 0 and pos[1] < x_max and 
#                     bool(tf.gather_nd(grid, pos))):
#                     return pos
...
#     # If no valid position found, return a random valid position
#     valid_indices = tf.where(grid)
#     random_idx = tf.random.uniform(shape=(), maxval=tf.shape(valid_indices)[0], dtype=tf.int32)
#     return tf.gather(valid_indices, random_idx)
...
#     if users == {}:
#         # Initialize users with better spread
#         valid_count = tf.shape(valid_indices)[0]
#         if valid_count < num_users:
#             raise ValueError(f"Not enough valid positions ({valid_count}) for requested users ({num_users})")
        
#         # Get initial random positions
#         random_ids = tf.random.uniform(shape=(num_users,), maxval=valid_count, dtype=tf.int32)
#         positions = tf.gather(valid_indices, random_ids)
...
#             # Initialize with a random direction and momentum
#             initial_direction = tf.random.uniform(shape=(2,), minval=-2, maxval=3, dtype=tf.int64)
...
#             momentum = users[f"ue{ue}"]["momentum"]
#             direction = tf.cast(users[f"ue{ue}"]["direction"][:2], tf.float32)
#             users[f"ue{ue}"]["momentum"] = 0.9 * momentum + 0.1 * direction
...
#         attempts = 0
#         original_move = move
#         while not valid_move and attempts < 5:
#             pos = start[0:2] + move
#             if (pos[1] >= x_max or pos[1] < 0 or 
#                 pos[0] >= y_max or pos[0] < 0 or 
#                 not bool(tf.gather_nd(grid, pos))):
#                 # If invalid, try a scaled version of the same direction
#                 scale = 0.8 - (attempts * 0.1)  # Reduce scale with each attempt
#                 move = tf.cast(tf.cast(original_move, tf.float32) * scale, tf.int64)
#                 attempts += 1
#             else:
#                 valid_move = True
